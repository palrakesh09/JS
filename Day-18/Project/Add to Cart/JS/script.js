...
const cartList = document.querySelector(".cart-tab .cart-list");
  ...
  cart.forEach(item => {
    ...
    row.innerHTML = `
      ...
      <div class="flex">
        ...
        <a href="#" class="quantity-btn increase" data-id="${item.id}"><i class="fa-solid fa-plus"></i></a>
...
document.addEventListener("click", (e) => {
  ...
  if (e.target.closest(".increase")) {
    ...
    item.quantity++; updateCart();
  }
  if (e.target.closest(".decrease")) {
    e.preventDefault();
    const id = e.target.closest(".decrease").dataset.id;
    const item = cart.find(p => p.id == id);
    item.quantity--; if (item.quantity <= 0) { removeFromCart(id); } else { updateCart(); }
  }
